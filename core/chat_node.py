...
from core.utils import parse_llm_response, merge_slots, extract_content
                ...
                try:
                    for chunk in self.chain.stream({
                        ...
                        if content is not None:
                            ...
                            buffer = buffer + str(chunk)
                            response_chunks.append(str(chunk))
                        # logger.info(f"BUFFER 상태: {repr(buffer)}")
                        if writer:
                            writer({"type": "stream", "message": content})
                    # None이 포함되지 않도록 join 전에 필터링
                    raw_output = "".join([c for c in response_chunks if c is not None])
                except Exception as e:
                    logger.error("슬롯 추천 스트리밍 중 오류 발생: %s", e)
                    raw_output = None
                logger.info("recommend response: %s", raw_output)
                llm_response = parse_llm_response(raw_output)
                recommended_value = default_value
                if isinstance(raw_output, dict) and 'recommended_slots' in raw_output:
...
class IntentDetector(BaseNode):
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("DetectedIntent raw_output: %s", result)
            ...
            logger.exception("IntentDetector 오류: %s", e)
...
class SlotCategoryExtractor(BaseNode):
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("slot, category extractor: %s", result)
            if isinstance(result, dict):
                state["slots"] = merge_slots(state.get("slots", {}), result.get("slots", {}))
                if not state.get("task_title") and result.get("task_title"):
            ...
            logger.exception("SlotCategoryExtractor 오류: %s", e)
    ...
    def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if state.get("awaiting_slot") and state.get("user_input"):
            ...
            try:
                ...
                logger.warning("MissingSlotAsker LLM slot extraction error: %s", e)
...
class BaseChainNode(BaseNode):
    ...
    def __call__(self, state, writer=None):
        ...
        try:
            response_chunks = []
            buffer = ""
            for chunk in self.chain.stream(invoke_args):
                content = extract_content(chunk)
                if content is not None:
                    buffer = buffer + content
                else:
            ...
            logger.error(f"{self.log_prefix} 중 오류가 발생했습니다.: %s", e)
...
class BaseScheduleGenerator(BaseChainNode):
    def __call__(self, state, writer=None):
        ...
        task_title = state.get('task_title', "")
        # task_title이 비어있으면 slots에서 추출 시도
        if not task_title:
            slots_task_title = slots_dict.get("task_title")
            if slots_task_title:
                task_title = slots_task_title
        self.extra_invoke_args = getattr(self, 'extra_invoke_args', [])
        state['slots_json'] = slots_json
        ...
        for arg in self.extra_invoke_args:
            invoke_args[arg] = state.get(arg, "")
        logger.info(f"{self.log_prefix} - 병합된 슬롯 정보: %s %s", task_title, slots_dict)
...
class QaGenerator(BaseChainNode):
    chain = qa_chain()
    log_prefix = "일반 질의"